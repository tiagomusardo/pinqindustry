	...
	private function addHeadDeclarations($id, $data = null) {
		...
		switch ($orientation) {
			case 'disabled':
				...
				break;
			case 'vertical':
				$width_extension = $this->params->autohide ? 0 : 20 + $this->params->thumb_width;
				$rules['height'] = $this->params->height.'px';
				...
				$selectors = array(
					...
					'#'.$id.' .showplus-thumbnails' => array(
						...
						$placement => -$width_extension.'px',  // align with exact top/bottom for auto-hide
						'top' => '0',
				...
				break;
			case 'horizontal':
				$height_extension = $this->params->autohide ? 0 : 20 + $this->params->thumb_height;
				...
				$selectors = array(
					...
					'#'.$id.' .showplus-thumbnails' => array(
						...
						$placement => -$height_extension.'px'  // align with exact left/right for auto-hide
					),
					'#'.$id.' .showplus-thumbnails ul' => array(
		...
		$options = array(
			...
			'overlay' => $this->params->overlay_position !== false,
